...
func TestPrivateKeyFromPem(t *testing.T) {
	f, err := os.Open(filepath.Join("..", "test_keys", "id_rsa"))
	if err != nil {
...
func TestPublicKeyFromPem(t *testing.T) {
	f, err := os.Open(filepath.Join("..", "test_keys", "id_rsa.pub"))
	if err != nil {
		t.Errorf("failed to open test_keys key: %q", err.Error())
	}
...
func TestToPem(t *testing.T) {
	pubb, err := ioutil.ReadFile(filepath.Join("..", "test_keys", "id_rsa.pub"))
	if err != nil {
	...
	privb, err := ioutil.ReadFile(filepath.Join("..", "test_keys", "id_rsa"))
	if err != nil {
		t.Error("failed to read test_keys key")
	}
	privf, err := os.Open(filepath.Join("..", "test_keys", "id_rsa"))
